...
def submit_workflows(
    jobs, wf_id=None, upload_to_rucio=True, update_db=True, force=True, debug=True, token=None
):
    """Submit workflows for list of jobs Jobs are grouped by env tag and
    context A single workflow is created for each group."""
...
def update_rules(jobs):
    ...
    for job in jobs:
        ...
        if not rucio_rule["exists"]:
            result = rc.AddRule(did, job.destination, lifetime=None)
            added.append(result)
    return added
